...
export function ProductTourStudyDemo({
    ...
    return (
      <DemoShell>
        <Card
          ...
          className="mt-8 flex min-h-56 items-center justify-center p-6"
        >
  ...
  return (
    <DemoShell>
      <div data-tour="tour-study-flashcard">
        <Card className="border-primary/15 bg-gradient-to-br from-card to-primary/5 p-5">
          ...
          {data.flashcard ? (
            <>
              <p className="mt-6 text-center text-[21px] font-bold leading-[1.4]">{data.flashcard.prompt}</p>
              <div
                className="mt-5 rounded-2xl border border-primary/15 bg-background/80 p-4 transition-[opacity,transform] duration-300 motion-reduce:transition-none"
                style={{
                  opacity: answerVisible ? 1 : 0,
                  visibility: answerVisible ? "visible" : "hidden",
                  transform: answerVisible ? "rotateY(0deg) scale(1)" : "rotateY(86deg) scale(.98)",
