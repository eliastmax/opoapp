...
async function readHidden(label: string): Promise<string> {
  ...
  return await new Promise<string>((resolve, reject) => {
    ...
    const onKeypress = (chunk: string, key: { name?: string; ctrl?: boolean }) => {
      if (key.ctrl && key.name === "c") { cleanup(); reject(new Error("Authentication cancelled.")); return; }
...
function parseArgs(args: readonly string[]): { command: Command; packagePath?: string } {
  assertSafeCliArgs(args);
  if (args.length === 1 && args[0] === "probe") return { command: "probe" };
  if ((args[0] === "preflight" || args[0] === "execute") && args.length === 3 && args[1] === "--package" && args[2]) {
    return { command: args[0], packagePath: args[2] };
  }
  throw new Error("Usage: bun run maintenance:celador-hardening -- probe | <preflight|execute> --package <approved-package.json>. No credential/JWT flags are supported.");
}

async function loadPackage(path: string, mode: "preflight" | "execute") {
  let raw: unknown;
  try { raw = JSON.parse(await readFile(path, "utf8")); } catch { throw new Error("Could not read a valid JSON hardening package."); }
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) throw new Error("Hardening package must be a JSON object.");
  ...
  if (mode === "execute") base.confirmation = executionConfirmation(fingerprint);
...
async function main(): Promise<void> {
  ...
  if (command === "probe") {
    payload = { package_id: ELI44_PROBE_PACKAGE_ID, mode: "probe", opposition_id: CELADOR_OPPOSITION_ID };
  } else {
    const pkg = await loadPackage(packagePath!, command);
    if (command === "execute") {
      stdout.write("\nWARNING: this command mutates existing Celador question content in place.\n");
      stdout.write("Do not continue without a separate Governance authorization for this exact package fingerprint.\n\n");
      const expected = executionConfirmation(pkg.package_fingerprint);
      const confirmation = await readLine(`Type ${expected} exactly: `);
      if (confirmation !== expected) throw new Error("Execution confirmation did not match; no RPC call made.");
    }
  ...
  try {
    ...
    const [{ data: profile, error: profileError }, { data: adminRows, error: adminError }] = await Promise.all([
      ...
      supabase.from("opposition_admins").select("opposition_id").eq("user_id", userId).eq("opposition_id", CELADOR_OPPOSITION_ID),
    ]);
    if (profileError || adminError) throw new Error("Authorization preflight failed.");
    if (profile?.active_opposition_id !== CELADOR_OPPOSITION_ID) throw new Error("Rejected: Celador SMS is not the active opposition.");
    if (!adminRows?.length) throw new Error("Rejected: authenticated user is not opposition_admin for Celador.");
...
main().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : "Unknown Celador hardening executor failure.";
