// @ts-expect-error bun:test is provided by the Bun test runtime
import { describe, expect, test } from "bun:test";
import {
  buildGate1Report,
  calculateFactoryCoverage,
  planDirectedQuestionGeneration,
  type ContentFactoryJob,
  type FactoryQuestionMetadata,
  type ProposedConcept,
  type ProposedStudyUnit,
} from "../content-factory";

const source = [{ label: "Temario.pdf", reference: "Temario.pdf, art. 1", pageStart: 1, pageEnd: 1 }];
const unit: ProposedStudyUnit = { code: "SMS-T01-U01", title: "Unidad", position: 1, sourceRefs: source };

function q(code: string): FactoryQuestionMetadata {
  return { code, active: true, stem: code };
}

function c(code: string, sourceCapacity?: ProposedConcept["sourceCapacity"]): ProposedConcept {
  return {
    code,
    unitCode: "SMS-T01-U01",
    title: code,
    description: "Descripción",
    position: Number(code.slice(-1)),
    sourceCapacity,
  };
}

function job(questions: FactoryQuestionMetadata[]): ContentFactoryJob {
  return {
    version: "1.0",
    oppositionCode: "auxiliar-administrativo-sms",
    topicNumber: 1,
    mode: "existing_bank",
    codePrefix: "SMS-T01",
    coverageThreshold: 4,
    source,
    existingQuestions: questions,
  };
}

describe("Content Factory source-limited coverage", () => {
  test("classifies a reached source ceiling as source_limited and plans no fake questions", () => {
    const questions = [q("SMS-T01-0001")];
    const concepts = [c("SMS-T01-C01", {
      status: "source_limited",
      sourceSupportedCeiling: 1,
      reason: "Atomic canonical source.",
    })];
    const coverage = calculateFactoryCoverage({
      questions,
      concepts,
      assignments: [{ questionCode: "SMS-T01-0001", primaryConceptCode: "SMS-T01-C01" }],
      threshold: 4,
    });

    expect(coverage.factoryConceptCoverage[0]).toEqual({
      conceptId: "SMS-T01-C01",
      status: "source_limited",
      primaryQuestionCount: 1,
      nominalThreshold: 4,
      nominalMissingPrimaryQuestions: 3,
      actionableMissingPrimaryQuestions: 0,
      sourceSupportedCeiling: 1,
      blockedAdditionalQuestions: 3,
    });
    expect(coverage.totalMissingQuestions).toBe(3);
    expect(coverage.totalActionableMissingQuestions).toBe(0);
    expect(coverage.totalBlockedBySourceCeiling).toBe(3);
    expect(planDirectedQuestionGeneration({
      coverage,
      codePrefix: "SMS-T01",
      usedQuestionCodes: questions.map((entry) => entry.code),
    })).toEqual([]);
  });

  test("generates only until an unreached source ceiling", () => {
    const questions = [q("SMS-T01-0001")];
    const concepts = [c("SMS-T01-C01", {
      status: "source_limited",
      sourceSupportedCeiling: 3,
      reason: "Three independent dimensions exist.",
    })];
    const coverage = calculateFactoryCoverage({
      questions,
      concepts,
      assignments: [{ questionCode: "SMS-T01-0001", primaryConceptCode: "SMS-T01-C01" }],
      threshold: 4,
    });

    expect(coverage.factoryConceptCoverage[0]).toMatchObject({
      status: "coverage_gap",
      actionableMissingPrimaryQuestions: 2,
      blockedAdditionalQuestions: 1,
    });
    expect(planDirectedQuestionGeneration({
      coverage,
      codePrefix: "SMS-T01",
      usedQuestionCodes: ["SMS-T01-0001"],
    })).toHaveLength(2);
  });

  test("keeps source_review_required separate from source_limited", () => {
    const questions = [q("SMS-T01-0001")];
    const concepts = [c("SMS-T01-C01", {
      status: "source_review_required",
      reason: "Canonical wording is ambiguous.",
    })];
    const coverage = calculateFactoryCoverage({
      questions,
      concepts,
      assignments: [{ questionCode: "SMS-T01-0001", primaryConceptCode: "SMS-T01-C01" }],
      threshold: 4,
    });
    expect(coverage.factoryConceptCoverage[0].status).toBe("source_review_required");
    expect(coverage.factoryConceptCoverage[0].actionableMissingPrimaryQuestions).toBe(0);
    expect(coverage.factoryConceptCoverage[0].blockedAdditionalQuestions).toBe(0);
  });

  test("reports actionable gaps, source_limited and source_review_required independently", () => {
    const questions = [q("SMS-T01-0001"), q("SMS-T01-0002"), q("SMS-T01-0003")];
    const concepts = [
      c("SMS-T01-C01"),
      c("SMS-T01-C02", { status: "source_limited", sourceSupportedCeiling: 1, reason: "Atomic." }),
      c("SMS-T01-C03", { status: "source_review_required", reason: "Ambiguous." }),
    ];
    const report = buildGate1Report({
      job: job(questions),
      units: [unit],
      concepts,
      assignments: questions.map((question, index) => ({
        questionCode: question.code,
        primaryConceptCode: concepts[index].code,
      })),
    });

    expect(report.summary.coverageGaps).toBe(1);
    expect(report.summary.sourceLimited).toBe(1);
    expect(report.summary.sourceReviewRequired).toBe(1);
    expect(report.summary.nominalQuestionsMissing).toBe(9);
    expect(report.summary.questionsNeeded).toBe(3);
    expect(report.summary.blockedAdditionalQuestions).toBe(3);
  });

  test("rejects a primary count above the declared source ceiling", () => {
    const questions = [q("SMS-T01-0001"), q("SMS-T01-0002")];
    const concepts = [c("SMS-T01-C01", {
      status: "source_limited",
      sourceSupportedCeiling: 1,
      reason: "Contradictory fixture.",
    })];
    expect(() => calculateFactoryCoverage({
      questions,
      concepts,
      assignments: questions.map((question) => ({ questionCode: question.code, primaryConceptCode: "SMS-T01-C01" })),
      threshold: 4,
    })).toThrow("primary question count exceeds sourceSupportedCeiling");
  });
});
