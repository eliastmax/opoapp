...
describe("private beta readiness contracts", () => {
  ...
  it("guards both curator routes and hides the curator CTA for learners", () => {
    expect(importer).toContain("beforeLoad: requireOppositionAdmin"); expect(questions).toContain("beforeLoad: requireOppositionAdmin"); expect(settings).toMatch(/\{isAdmin\s*&&/);
  });
