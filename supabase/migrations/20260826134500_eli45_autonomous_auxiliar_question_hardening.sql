...
--
-- Goal: allow the trusted internal Postgres maintenance runtime to execute
-- narrowly-scoped Auxiliar question-hardening packages without requiring a
-- human Supabase Auth login for every topic. Learner/app RLS remains unchanged.
--
-- This path is deliberately separate from the ELI-43 authenticated operator CLI.
-- It is internal-only, package/fingerprint guarded, topic locked, update-in-place
-- only, and cannot mutate Celador or non-question catalog surfaces.

do $$
begin
  if not exists (select 1 from pg_roles where rolname = 'auxiliar_question_hardening_executor') then
    create role auxiliar_question_hardening_executor nologin noinherit bypassrls;
  end if;
end
$$;

alter role auxiliar_question_hardening_executor nologin noinherit bypassrls;
...
grant usage on schema catalog_maintenance_private to auxiliar_question_hardening_executor;
...
grant select, insert on table catalog_maintenance_private.auxiliar_hardening_audit to auxiliar_question_hardening_executor;
grant usage, select on sequence catalog_maintenance_private.auxiliar_hardening_audit_audit_id_seq to auxiliar_question_hardening_executor;

grant usage on schema public to auxiliar_question_hardening_executor;
grant select on public.questions, public.topics to auxiliar_question_hardening_executor;
grant update (pregunta, opcion_a, opcion_b, opcion_c, opcion_d, respuesta_correcta, explicacion, nivel_pedagogico, tipo_trampa)
  on public.questions to auxiliar_question_hardening_executor;
...
grant execute on function catalog_maintenance_private.auxiliar_question_fingerprint(uuid)
  to postgres, auxiliar_question_hardening_executor;

-- Extend the existing catalog trigger with one additional internal role branch.
-- The existing authenticated and Celador Factory behavior remains unchanged.
...
begin
  if current_user='auxiliar_question_hardening_executor' then
...
begin
  if session_user <> 'postgres' then
    raise exception 'ELI-45 internal hardening executor is restricted to the trusted Postgres maintenance runtime'
      using errcode='42501';
  end if;

  if p_package is null or jsonb_typeof(p_package) <> 'object' then
    raise exception 'ELI-45 package must be a JSON object' using errcode='22023';
  end if;
  ...
  if coalesce(cardinality(v_unknown_keys),0) > 0 then
    raise exception 'ELI-45 package contains unsupported keys: %',array_to_string(v_unknown_keys,', ')
      using errcode='22023';
  end if;

  v_package_id:=nullif(p_package->>'package_id','');
  v_governance_ref:=nullif(p_package->>'governance_ref','');
  v_mode:=nullif(p_package->>'mode','');
  v_package_fingerprint:=lower(nullif(p_package->>'package_fingerprint',''));
  v_changes:=p_package->'changes';

  begin
    v_opposition_id:=nullif(p_package->>'opposition_id','')::uuid;
    v_topic_id:=nullif(p_package->>'topic_id','')::uuid;
  exception when invalid_text_representation then
    raise exception 'ELI-45 package contains an invalid UUID' using errcode='22023';
  end;

  if v_package_id is distinct from 'auxiliar_question_hardening_v1' then
    raise exception 'ELI-45 package_id is not allowlisted' using errcode='22023';
  end if;
  if v_governance_ref is null or v_governance_ref !~ '^ELI-[0-9]+$' then
    raise exception 'ELI-45 requires a Linear governance_ref such as ELI-41' using errcode='22023';
  end if;
  if v_mode is null or v_mode not in ('preflight','execute') then
    raise exception 'ELI-45 mode must be preflight or execute' using errcode='22023';
  end if;
  if v_opposition_id is distinct from v_auxiliar_opposition_id then
    raise exception 'ELI-45 is restricted to Auxiliar Administrativo SMS' using errcode='42501';
  end if;
  if v_topic_id is null or not exists(
    select 1 from public.topics t where t.id=v_topic_id and t.opposition_id=v_auxiliar_opposition_id
  ) then
    raise exception 'ELI-45 topic is outside Auxiliar' using errcode='42501';
  end if;
  if v_package_fingerprint is null or v_package_fingerprint !~ '^[0-9a-f]{32}$' then
    raise exception 'ELI-45 package_fingerprint must be lowercase MD5 hex' using errcode='22023';
  end if;

  v_computed_package_fingerprint:=md5((p_package-'package_fingerprint')::text);
  if v_package_fingerprint is distinct from v_computed_package_fingerprint then
    raise exception 'ELI-45 package fingerprint mismatch' using errcode='P0001';
  end if;

  if v_changes is null or jsonb_typeof(v_changes)<>'array' then
    raise exception 'ELI-45 changes must be an array' using errcode='22023';
  end if;
  v_change_count:=jsonb_array_length(v_changes);
  if v_change_count < 1 or v_change_count > 500 then
    raise exception 'ELI-45 package row count must be between 1 and 500' using errcode='22023';
  end if;

  if exists(
    select 1 from (
      select change->>'id' value, count(*) n
      from jsonb_array_elements(v_changes) change
      group by change->>'id'
      having count(*)>1
    ) duplicates
  ) then
    raise exception 'ELI-45 package contains duplicate question ids' using errcode='22023';
  end if;
  if exists(
    select 1 from (
      select change->>'codigo' value, count(*) n
      from jsonb_array_elements(v_changes) change
      group by change->>'codigo'
      having count(*)>1
    ) duplicates
  ) then
    raise exception 'ELI-45 package contains duplicate question codes' using errcode='22023';
  end if;
  ...
  loop
    if jsonb_typeof(v_change)<>'object' then
      raise exception 'ELI-45 each change must be an object' using errcode='22023';
    end if;

    select array_agg(key order by key)
    into v_unknown_keys
    from jsonb_object_keys(v_change) as keys(key)
    where key not in (
      'id','codigo','expected_fingerprint','pregunta','opcion_a','opcion_b','opcion_c','opcion_d',
      'respuesta_correcta','explicacion','nivel_pedagogico','tipo_trampa'
    );
    if coalesce(cardinality(v_unknown_keys),0)>0 then
      raise exception 'ELI-45 change contains unsupported keys: %',array_to_string(v_unknown_keys,', ')
        using errcode='22023';
    end if;

    begin
      v_question_id:=nullif(v_change->>'id','')::uuid;
    exception when invalid_text_representation then
      raise exception 'ELI-45 change contains invalid question id' using errcode='22023';
    end;
    v_codigo:=nullif(v_change->>'codigo','');
    v_expected_fingerprint:=lower(nullif(v_change->>'expected_fingerprint',''));
    v_answer:=upper(nullif(v_change->>'respuesta_correcta',''));
    v_level:=lower(nullif(v_change->>'nivel_pedagogico',''));
    v_trap:=nullif(v_change->>'tipo_trampa','');

    if v_question_id is null or v_codigo is null or v_expected_fingerprint is null then
      raise exception 'ELI-45 change identity/fingerprint fields are required' using errcode='22023';
    end if;
    if v_expected_fingerprint !~ '^[0-9a-f]{32}$' then
      raise exception 'ELI-45 expected_fingerprint must be lowercase MD5 hex for %',v_codigo using errcode='22023';
    end if;
    if coalesce(btrim(v_change->>'pregunta'),'')=''
       or coalesce(btrim(v_change->>'opcion_a'),'')=''
       or coalesce(btrim(v_change->>'opcion_b'),'')=''
       or coalesce(btrim(v_change->>'opcion_c'),'')=''
       or coalesce(btrim(v_change->>'opcion_d'),'')=''
       or coalesce(btrim(v_change->>'explicacion'),'')='' then
      raise exception 'ELI-45 hardened text fields cannot be empty for %',v_codigo using errcode='22023';
    end if;
    if v_answer not in ('A','B','C','D') then
      raise exception 'ELI-45 respuesta_correcta must be A/B/C/D for %',v_codigo using errcode='22023';
    end if;
    if v_level not in ('aprendizaje','consolidacion','tribunal') then
      raise exception 'ELI-45 nivel_pedagogico invalid for %',v_codigo using errcode='22023';
    end if;
    if v_trap is not null and (length(v_trap)>64 or v_trap !~ '^[a-z0-9_]+$') then
      raise exception 'ELI-45 tipo_trampa invalid for %',v_codigo using errcode='22023';
    end if;
    ...
    if not found then
      raise exception 'ELI-45 question id not found: %',v_question_id using errcode='P0001';
    end if;
    if not v_question.activa
       or v_question.opposition_id is distinct from v_auxiliar_opposition_id
       or v_question.topic_id is distinct from v_topic_id
       or v_question.codigo is distinct from v_codigo then
      raise exception 'ELI-45 question identity/scope mismatch for %',v_codigo using errcode='P0001';
    end if;
    ...
    if v_current_fingerprint is distinct from v_expected_fingerprint then
      raise exception 'ELI-45 stale package detected for %',v_codigo using errcode='P0001';
    end if;

    if v_mode='execute' then
      update public.questions q
      set pregunta=v_change->>'pregunta',
          ...
          tipo_trampa=v_trap
      where q.id=v_question_id
        and q.codigo=v_codigo
        and q.opposition_id=v_auxiliar_opposition_id
        and q.topic_id=v_topic_id
        and q.activa;

      get diagnostics v_affected=row_count;
      if v_affected<>1 then
        raise exception 'ELI-45 update count mismatch for %',v_codigo using errcode='P0001';
      end if;
      ...
      if not exists(
        ...
        where q.id=v_question_id
          ...
          and q.topic_id=v_topic_id
          and q.activa
          and q.pregunta is not distinct from v_change->>'pregunta'
          and q.opcion_a is not distinct from v_change->>'opcion_a'
          and q.opcion_b is not distinct from v_change->>'opcion_b'
          and q.opcion_c is not distinct from v_change->>'opcion_c'
          and q.opcion_d is not distinct from v_change->>'opcion_d'
          and q.respuesta_correcta::text is not distinct from v_answer
          and q.explicacion is not distinct from v_change->>'explicacion'
          and q.nivel_pedagogico is not distinct from v_level
          and q.tipo_trampa is not distinct from v_trap
      ) then
        raise exception 'ELI-45 postcondition mismatch for %; transaction rolled back',v_codigo using errcode='P0001';
      end if;
    end if;
  end loop;
  ...
  if v_mode='preflight' then
    return jsonb_build_object(
      ...
      'governance_ref',v_governance_ref,
      'opposition_id',v_auxiliar_opposition_id,
      'topic_id',v_topic_id,
      'row_count',v_change_count,
      'package_fingerprint',v_package_fingerprint,
      'academic_writes',0,
      'executor','internal_governance'
    );
  end if;

  insert into catalog_maintenance_private.auxiliar_hardening_audit(
    package_id,governance_ref,opposition_id,topic_id,package_fingerprint,row_count,executor_role
  ) values (
    v_package_id,v_governance_ref,v_auxiliar_opposition_id,v_topic_id,v_package_fingerprint,v_change_count,current_user
  );
  ...
  return jsonb_build_object(
    ...
    'row_count',v_change_count,
    'package_fingerprint',v_package_fingerprint,
    'academic_writes',v_change_count,
    'executor','internal_governance'
  );
end;
$function$;

alter function catalog_maintenance_private.execute_auxiliar_question_hardening(jsonb)
  owner to auxiliar_question_hardening_executor;
...
-- No app/user grants are added. Existing ELI-43 remains unchanged and available.
