-- V4 evidence timing correction.
-- test_answers rows can pre-exist test completion. Retention and recency must be
-- anchored to the completed session timestamp, not merely row creation time.
  ...
  COALESCE((
    SELECT jsonb_agg(
      jsonb_build_object(
        ...
        'answeredAt', COALESCE(test.fecha_finalizacion, answer.created_at),
      ...
      ORDER BY COALESCE(test.fecha_finalizacion, answer.created_at), answer.id
    )
    FROM public.test_answers answer
    JOIN public.tests test
      ON test.id = answer.test_id
     AND test.user_id = viewer.user_id
...
COMMENT ON FUNCTION public.get_my_v4_concept_evidence(uuid) IS
  'Returns authenticated learner evidence for the V4 mastery evaluator. Completed test time is the canonical question-evidence timestamp; targeted V4 selections override primary attribution and preserve retention checkpoint metadata.';
