-- V3.4: adapt and validate the weekly roadmap without changing its priority formula.
-- Distinguishes unknown vs low self-assessment, exposes an explicit no-question
-- terminal state, and preserves calm re-planning with at most one session per
-- remaining configured day.
...
ranked_topics AS (
  SELECT
    ...
    CASE
      ...
      WHEN assessment_weight > 0 AND estimated_percentage IS NULL THEN 'initial_unknown'
      WHEN assessment_weight > 0 AND estimated_percentage <= 25 THEN 'initial_low'
    ...
    CASE
      ...
      WHEN assessment_weight > 0 AND estimated_percentage IS NULL
        THEN 'No sabías valorarlo y aún hay poca evidencia real.'
      WHEN assessment_weight > 0 AND estimated_percentage <= 25
        THEN 'Lo valoraste bajo y aún hay poca evidencia real.'
...
topic_count AS (
  SELECT count(*)::integer AS value FROM ranked_topics
)
SELECT
...
SELECT
  ...
  CASE
    WHEN plan.outstanding_sessions = 0 THEN 'week_complete'
    WHEN cardinality(plan.remaining_dates) = 0 THEN 'no_days_remaining'
    WHEN NOT EXISTS (SELECT 1 FROM ranked_topics) THEN 'no_questions_available'
    ELSE 'no_days_remaining'
  END AS reason_code,
  CASE
    WHEN plan.outstanding_sessions = 0 THEN 'Esta semana ya has alcanzado el ritmo que configuraste.'
    WHEN cardinality(plan.remaining_dates) = 0
      THEN 'No quedan días configurados esta semana; la ruta se recalculará sin acumular sesiones.'
    WHEN NOT EXISTS (SELECT 1 FROM ranked_topics)
      THEN 'No hay preguntas activas disponibles en esta oposición para preparar una sesión.'
  ...
  END AS exam_guidance
FROM plan
WHERE plan.outstanding_sessions = 0
   OR cardinality(plan.remaining_dates) = 0
   OR NOT EXISTS (SELECT 1 FROM ranked_topics)
