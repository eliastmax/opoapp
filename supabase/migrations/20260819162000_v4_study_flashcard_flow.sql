...
alter table public.flashcard_reviews
  drop constraint if exists flashcard_reviews_known_streak_after_check;
alter table public.flashcard_reviews
  add constraint flashcard_reviews_known_streak_after_check
  check (known_streak_after >= 0);

alter table public.flashcard_reviews
  drop constraint if exists flashcard_reviews_scheduled_delay_minutes_check;
alter table public.flashcard_reviews
...
-- Study progress and card-review evidence must be written through validated RPCs.
...
declare
  ...
  v_unit public.study_units%rowtype;
  v_progress public.study_unit_progress%rowtype;
  v_concepts jsonb;
  v_cards jsonb;
begin
  ...
  values (
    v_user_id, v_opposition_id, v_unit.id, now(), now(), now()
  )
  ...
  where concept.study_unit_id = v_unit.id
  ...
  return jsonb_build_object(
    'unit', jsonb_build_object(
      'id', v_unit.id,
      'code', v_unit.code,
      'topicId', v_unit.topic_id,
      'title', v_unit.title,
      'position', v_unit.position,
      'estimatedMinutes', v_unit.estimated_minutes,
      'studySummary', v_unit.study_summary,
      'examKeys', v_unit.exam_keys,
      'confusions', v_unit.confusions,
      'traps', v_unit.traps,
      'mnemonics', v_unit.mnemonics,
      'sourceRefs', v_unit.source_refs
    ),
    'progress', jsonb_build_object(
      'firstOpenedAt', v_progress.first_opened_at,
      'lastOpenedAt', v_progress.last_opened_at,
      'completedAt', v_progress.completed_at,
      'completionCount', v_progress.completion_count
    ),
    'concepts', v_concepts,
    'flashcards', v_cards
  );
...
begin
  ...
  select card.*, concept.study_unit_id
  into v_card, v_unit_id
  from public.flashcards card
  join public.concepts concept on concept.id = card.concept_id
...
create or replace function public.review_my_v4_flashcard(
  p_flashcard_id uuid,
  p_rating text
)
returns jsonb
