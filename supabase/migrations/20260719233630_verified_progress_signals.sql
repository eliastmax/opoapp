-- V2 verified progress v1: only factual changes backed by separated, comparable evidence.
      ...
      WHERE latest.correcta IS TRUE
        AND EXISTS (
          ...
          WHERE prior_answer.user_id = latest.user_id
            ...
            AND prior_answer.correcta IS FALSE
            AND prior_test.fecha_finalizacion
              <= latest.fecha_finalizacion - interval '1 day'
        )
    )::integer AS corrected_failures_30d
  FROM latest_answers AS latest
  WHERE latest.fecha_finalizacion >= now() - interval '30 days'
...
comparable_pairs AS (
  SELECT
    latest.topic_id,
    latest.question_id,
    previous.correcta AS baseline_correct,
    latest.correcta AS current_correct,
    previous.test_id AS baseline_test_id,
  ...
  JOIN LATERAL (
    ...
    WHERE prior_answer.user_id = latest.user_id
      ...
      AND prior_test.fecha_finalizacion
        <= latest.fecha_finalizacion - interval '7 days'
...
COMMENT ON FUNCTION public.get_verified_progress_summary() IS
  'Returns factual 30-day corrections, seven-day retention and like-for-like accuracy changes; comparison requires 10 questions and two sessions per side.';
